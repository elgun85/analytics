...
@section('title','ƏDV-dən azad sənədləşmələr')
@section('content')
<main id="main" class="main">
    ...
    <section class="section">
        <div class="row">
            <div class="card">
                <div class="card-body">
                    <form  action="{{route('edvs')}}" method="get" name="formdan">
                        @csrf
                        <div class="row mb-3">
                            <div class="col-sm-2">
                    ...
                    <table id="example1" class="table table-bordered table-striped ">
                        ...
                        @foreach($resultsPay as $itemPay)
                            <tr class="text-center">
                                <td class="text-left">
                                    {{ str_replace($bul, $dey, $itemPay->adqurum) }}
                                </td>
                                <td>{{ $itemPay->kodmhm }}</td>
                                <td>{{ $itemPay->kodqurum }}</td>
                                <td>{{ $itemPay->kodxidmet }}</td>
                                <td>{{ $itemPay->kateqor }}</td>
                                <td>{{ $itemPay->summa }}</td>
                            </tr>
                        @endforeach
                        <tr class="text-center">
